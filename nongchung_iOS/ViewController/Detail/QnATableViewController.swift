...
class QnATableViewController: UITableViewController {
    private func getData() -> [QnAData?] {
        ...
        let second_content = [AnswerData(content: "우리 농촌 활동 함께해요 특집이 게시되었습니다. 특집은 농활참여자들이 학생일 경우 10%할인 됩니다. 많은 관심 부탁드립니다. *특집 농활* 1.우도환 완전 좋아좋아 짱이야 농장 2.서강준 짱멋져 농장 3.박서준이 최고지 농장")]
        ...
        let sixth_content = [AnswerData(content: "")]
        let sixth = QnAData(title: "*더 자세한 문의를 원한다면 010 0000 (농활청춘 담당자)로 연락해주세요!", contents: sixth_content)
...
//
//                // Set the cell's data
//                expansionCell.answearLabel.text = rowData.contents?[contentIndex].content
//
